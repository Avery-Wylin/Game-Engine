...
void main(void){
    ...
    for(int i=0;i<4;i++){
        ...
        vec3 reflected = reflect(-unitToLight,surfaceNormal);
        float rawSpecular = dot(reflected,unitToCamera);
        rawSpecular = max(rawSpecular,0.0);
        float shinedSpecular = pow(rawSpecular,shine);
        combinedSpecular += (shinedSpecular * specular * lightColour[i])/attenuationFactor;
